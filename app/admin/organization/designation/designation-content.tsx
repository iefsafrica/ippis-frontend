...
} from "@/services/hooks/organizations/designation/useDesignation"
import type {
...
const defaultFormState: DesignationFormState = {
  company_code: "",
  title: "",
  description: "",
  status: "active",
...
export default function DesignationContent() {
  ...
  const activeCount = useMemo(
    () => designations.filter((row) => row.status?.toLowerCase() === "active").length,
    [designations]
  )
  const inactiveCount = useMemo(
    () => designations.filter((row) => row.status?.toLowerCase() === "inactive").length,
    [designations]
  )
  const recentCount = useMemo(() => {
    const thirtyDaysAgo = new Date()
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30)
    return designations.filter((row) => (row.created_at ? new Date(row.created_at) >= thirtyDaysAgo : false)).length
  }, [designations])

  const searchFields = [
    { name: "title", label: "Designation Title", type: "text" as const },
  ...
  const handleCreateDesignation = async () => {
    if (!form.title.trim() || !form.company_code.trim()) {
      toast.error("Designation title and company code are required")
      return
    }
  ...
  const handleDeleteDesignation = async () => {
    if (!selectedDesignation) return
    try {
      await deleteDesignationMutation.mutateAsync(selectedDesignation.id)
  ...
  const columns = [
    {
      ...
      sortable: true,
      render: (value: string, row: Designation) => (
        <div className="flex items-center">
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs text-gray-500">{row.company_code || "N/A"}</div>
          </div>
        </div>
      ),
    },
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[320px] gap-3">
        <p className="text-red-600 text-sm">{error.message || "Failed to load designations"}</p>
  ...
  return (
    <div className="container mx-auto px-4 py-6">
        ...
        <div className="flex items-center space-x-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-gray-600 mt-1 text-sm sm:text-base break-words">
              Manage organization designation records
              <span className="ml-1 sm:ml-2 text-sm text-gray-500">({designations.length} records)</span>
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Card className="border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-gray-500">Active Designation</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-gray-500">Inactive Designation</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-gray-500">Recent Designation</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-gray-500">Total Designation</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>Create a new designation record.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="designation-company-code">Company Code</Label>
              <Input
                id="designation-company-code"
            ...
            <div className="space-y-2">
              <Label htmlFor="designation-title">Title</Label>
              <Input
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateDesignation} disabled={createDesignationMutation.isPending}>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>Update designation details for {selectedDesignation?.title || "selected designation"}.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="edit-designation-company-code">Company Code</Label>
              <Input
                id="edit-designation-company-code"
                value={form.company_code}
                disabled
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="edit-designation-title">Title</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-designation-description">Description</Label>
              <Input
                id="edit-designation-description"
                value={form.description}
                onChange={(e) => setForm((prev) => ({ ...prev, description: e.target.value }))}
              />
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpdateDesignation} disabled={updateDesignationMutation.isPending}>
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center space-x-4">
                <div className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                  <BriefcaseBusiness className="h-6 w-6 text-gray-700" />
                </div>
                <div>
                  ...
                  <DialogDescription className="text-gray-600 mt-1">
                    View designation information for {selectedDesignation?.title}
                  </DialogDescription>
                </div>
              </div>
              <Button
            ...
            <div className="px-8 py-6 max-h-[70vh] overflow-y-auto">
              <div className="space-y-8">
                <div className="space-y-4">
                  <h3 className="text-sm font-medium text-gray-900 uppercase tracking-wide">Designation Information</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-4">
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-900">{selectedDesignation.title}</p>
                        </div>
                        <div>
                          <div className="flex items-center mb-2">
                            <Hash className="h-4 w-4 text-gray-500 mr-2" />
                            <span className="text-xs text-gray-500 font-medium">Designation ID</span>
                          </div>
                          <p className="text-sm font-medium text-gray-900 font-mono">{selectedDesignation.id}</p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          {getStatusBadge(selectedDesignation.status)}
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="pt-6 border-t border-gray-200">
                  <h3 className="text-sm font-medium text-gray-900 uppercase tracking-wide mb-6">More Details</h3>
                  <div className="space-y-6">
                    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                      <p className="text-sm text-gray-700"><span className="font-medium">Company Code:</span> {selectedDesignation.company_code || "N/A"}</p>
                      <p className="text-sm text-gray-700 mt-2"><span className="font-medium">Company Name:</span> {selectedDesignation.company_name || "N/A"}</p>
                      <p className="text-sm text-gray-700 mt-2"><span className="font-medium">Description:</span> {selectedDesignation.description || "N/A"}</p>
                    </div>
                    ...
                    <div className="pt-2 border-t border-gray-200">
                      ...
                      <p className="text-sm text-gray-600">
                        {selectedDesignation.created_at ? new Date(selectedDesignation.created_at).toLocaleString() : "N/A"}
                      </p>
                    </div>
                  </div>
                </div>
      ...
      <DeleteConfirmationDialog
        ...
        description={`Are you sure you want to delete ${selectedDesignation?.title || "this designation"}?`}
        itemName={selectedDesignation?.title || "this designation"}
        isLoading={deleteDesignationMutation.isPending}
      />
    </div>
  )
}
