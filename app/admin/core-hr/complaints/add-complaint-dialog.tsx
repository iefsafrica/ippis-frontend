...
import { Loader2, User, Building, Calendar, AlertCircle, ChevronDown, Hash, AlertTriangle, CheckCircle, Briefcase } from "lucide-react"
import { toast } from "sonner"
import { useAllEmployees } from "@/services/hooks/hr-core/usePromotions"
import type { CreateEmployeeComplaintRequest } from "@/types/hr-core/employee-complaints"

interface AddComplaintDialogProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (data: CreateEmployeeComplaintRequest) => Promise<void>
  isLoading?: boolean
...
export function AddComplaintDialog({ 
  ...
  const [formData, setFormData] = useState<CreateEmployeeComplaintRequest>({
    employee_id: "",
    employee_name: "",
    complaint: "",
    department: "",
  ...
  const handleSelectChange = (name: keyof CreateEmployeeComplaintRequest, value: string) => {
    setFormData((prev) => ({ ...prev, [name]: value }))
  ...
  const handleEmployeeSelect = (selectedEmployeeId: string) => {
    const employee = dropdownEmployees.find((emp) => emp.id === selectedEmployeeId)
    if (employee) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const loadingToast = toast.loading("Creating complaint record...")

    try {
      await onSubmit(formData)
      
      toast.success("Complaint record created successfully!", {
        id: loadingToast,
        duration: 3000,
      })
      ...
      toast.error("Failed to create complaint record. Please try again.", {
        id: loadingToast,
        duration: 4000,
      })
    } finally {
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <DialogDescription className="text-gray-600 mt-1">
                  Create a complaint record for an employee
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
              ...
              <div className="pt-6 border-t border-gray-200">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="submitted_on" className="text-sm font-medium text-gray-700 mb-2 block">
                        Submission Date
                      </Label>
